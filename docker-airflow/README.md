...
Go to Admin -> Connections and Edit "postgres_default" to set these values (equivalent to values in airflow.cfg/docker-compose*.yml) :
- Host : postgres
- Schema : airflow
- Login : airflow
- Password : airflow
...
Airflow allows for custom user-created plugins which are typically found in the `${AIRFLOW_HOME}/plugins` folder. Documentation on plugins can be found [here](https://airflow.apache.org/plugins.html)
...
- The entrypoint.sh script executes the pip install command (with --user option)
